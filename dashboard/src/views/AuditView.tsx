...
import { LiveAudit } from '../components/live'

export default function AuditView() {
  const { engine, live } = useSemi()

  const auditEvents = useMemo(
    () => engine.state.events.filter((e) => e.origin === 'audit').slice(0, 40),
    [engine.state.events],
  )

  const active = engine.state.rows.find((r) => r.stage === 'audit')
  const totalChecks = engine.state.rows.reduce((a, r) => a + r.audits.length, 0)
  const passChecks = engine.state.rows.reduce(
    (a, r) => a + r.audits.filter((x) => x.state === 'pass').length,
    0,
  )
  const failChecks = engine.state.rows.reduce(
    (a, r) => a + r.audits.filter((x) => x.state === 'fail').length,
    0,
  )
