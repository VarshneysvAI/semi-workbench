...
import { ArrowRight, GitBranch, History, Zap } from 'lucide-react'
...
function WeightBar({ label, source, weight, active }: { label: string; source: string; weight: number; active?: boolean }) {
  return (
    <div className={cn('rounded-xl border p-3.5', active ? 'border-emerald-400/30 bg-emerald-400/[0.05]' : 'border-white/[0.06] bg-white/[0.02]')}>
      ...
      <div className="mt-1.5 font-mono font-num text-[12px] font-semibold text-slate-200">
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }}>
        <div className="mb-6">
          ...
          <p className="mt-0.5 text-[12px] text-slate-500">
            watss / wv-1011 / thread_standard — an unresolved signal now resolved by a matched ledger precedent
          </p>
        </div>
          ...
          <Panel className="p-5">
            ...
            <div className="rounded-xl border border-white/[0.06] bg-white/[0.01] p-4">
              <div className="flex items-center gap-2 text-[12px] text-slate-400">
                <span className="font-mono text-slate-300">0.54</span>
                <ArrowRight className="h-3.5 w-3.5" />
                <span className="rounded-md bg-amber-400/10 px-2 py-0.5 font-mono font-semibold text-amber-300">REFUSED</span>
              </div>
              <p className="mt-2 text-[11px] leading-relaxed text-slate-500">
                Below the 0.70 threshold with no family boost — SEMI would refuse this value rather than guess.
              </p>
            </div>
          </Panel>

          <Panel className="border-emerald-400/20 bg-emerald-400/[0.03] p-5">
            <div className="mb-3 flex items-center gap-2">
            ...
            <div className="rounded-xl border border-emerald-400/20 bg-emerald-400/[0.05] p-4">
              <div className="flex items-center gap-2 text-[12px]">
                <span className="font-mono text-slate-200">0.54</span>
                <span className="font-mono text-emerald-300">+0.30</span>
                <ArrowRight className="h-3.5 w-3.5" />
                <span className="rounded-md bg-emerald-400/15 px-2 py-0.5 font-mono font-semibold text-emerald-300">
                  ACCEPTED · 0.84
                </span>
              </div>
              <p className="mt-2 text-[11px] leading-relaxed text-slate-500">
                Same conflict signature <span className="font-mono text-slate-400">NPT-vs-BSPT</span> resolved as NPT across 3
                manufacturers. <span className="text-emerald-300">ledger_changed_outcome = true</span> — the system changed its
                output because it remembered.
              </p>
            </div>
          </Panel>
        </div>
        ...
        <Panel className="p-5">
          <div className="mb-4 flex items-center gap-2">
            ...
            <SectionTitle>Precedent ledger — recent resolutions</SectionTitle>
            <StatusPill label="auto-apply on" tone="emerald" pulse className="ml-auto" />
          </div>
          <div className="divide-y divide-white/[0.04]">
            {LEDGER_EVENTS.map((ev) => (
              <div key={`${ev.at}-${ev.sku}`} className="flex items-center gap-3 py-2.5 text-[12px]">
                <span className="font-mono font-num text-[11px] text-slate-600">{ev.at}</span>
                <span className="w-40 font-mono text-slate-300">{ev.signature}</span>
