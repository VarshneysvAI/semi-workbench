...
export default function LedgerView() {
  ...
  const ledgerEvents = useMemo(
    () => engine.state.events.filter((e: any) => e.origin === 'ledger').slice(0, 14),
    [engine.state.events],
  )
  ...
  return (
    <div className="mx-auto max-w-[1100px] space-y-5 p-5">
      ...
      <div className="grid grid-cols-2 gap-3 xl:grid-cols-4">
        ...
        <LedgerStat label="rows processed" value={engine.state.rows.length} tone="violet" />
      </div>
          ...
          <table className="w-full border-collapse text-left text-[12px]">
            ...
            <tbody>
              {engine.state.ledger.length ? (
                engine.state.ledger.map((r: any, i: number) => (
